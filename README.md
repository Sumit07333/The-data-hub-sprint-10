...
```
*Note: If `MONGO_URI` is not yet set, the server still boots safely, logging a clear warning and returning HTTP 503 on database routes instead of crashing.*
Local base URL:
```
http://localhost:5000
```
...
    "authorId": { "_id": "64f8a1b2c3d4e5f6a7b8c9d0", "name": "Jane Doe", "email": "jane@example.com" },
